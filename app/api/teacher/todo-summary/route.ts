...
import { buildTeacherTodoSummary } from '@/services/todoSummary';
...
export async function GET(req: NextRequest) {
  ...
  try {
    const summary = await buildTeacherTodoSummary(auth.session.id);
    return NextResponse.json(summary);
  } catch (error) {
    const siteErr = trySiteDbReadErrorResponse(error, req);
    if (siteErr) return siteErr;
    console.error('[todo-summary/teacher]', error);
